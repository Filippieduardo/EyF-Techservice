"use client";
import { useState } from "react";
import { InformeLayout } from "@/components/informe-layout";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { formatCurrency } from "@/lib/constants";

interface Row { estado: string; cantidad: number; monto: number; porcentaje: number; }

const LABEL: Record<string, string> = { PENDIENTE: "Pendiente", APROBADO: "Aprobado", RECHAZADO: "Rechazado", VENCIDO: "Vencido" };
const mesAtras = () => { const d = new Date(); d.setMonth(d.getMonth() - 1); return d.toISOString().split("T")[0]; };
const hoy = () => new Date().toISOString().split("T")[0];
const fmt = (s: string) => s ? new Date(s).toLocaleDateString("es-AR", { day: "2-digit", month: "2-digit", year: "numeric" }) : "";

export default function Page() {
  const [desde, setDesde] = useState(mesAtras());
  const [hasta, setHasta] = useState(hoy());
  const [rows, setRows] = useState<Row[]>([]);
  const [total, setTotal] = useState(0);
  const [loading, setLoading] = useState(false);
  const [buscado, setBuscado] = useState(false);

  async function buscar() {
    setLoading(true);
    const res = await fetch(`/api/informes/presup-aprobacion?desde=${desde}&hasta=${hasta}`);
    const data = await res.json();
    setRows(data.rows); setTotal(data.total); setBuscado(true); setLoading(false);
  }

  const aprobados = rows.find(r => r.estado === "APROBADO")?.cantidad ?? 0;
  const tasaAprobacion = total > 0 ? Math.round((aprobados / total) * 100) : 0;
  const filtrosTexto = buscado ? `Período: ${fmt(desde)} al ${fmt(hasta)}  ·  Total: ${total} presupuestos  ·  Tasa de aprobación: ${tasaAprobacion}%` : undefined;

  const printTable = (
    <table className="data">
      <thead><tr>
        <th>Estado</th><th className="center">Cantidad</th><th className="center">Porcentaje</th><th className="right">Monto total</th>
      </tr></thead>
      <tbody>
        {rows.map((r, i) => (
          <tr key={i} className={i % 2 === 1 ? "alt" : ""}>
            <td>{LABEL[r.estado] ?? r.estado}</td>
            <td className="center" style={{ fontWeight: "bold" }}>{r.cantidad}</td>
            <td className="center">{r.porcentaje}%</td>
            <td className="right">{formatCurrency(r.monto)}</td>
          </tr>
        ))}
      </tbody>
      <tbody><tr className="total">
        <td>TOTAL</td>
        <td className="center">{total}</td>
        <td className="center">100%</td>
        <td className="right">{formatCurrency(rows.reduce((s, r) => s + r.monto, 0))}</td>
      </tr></tbody>
    </table>
  );

  return (
    <InformeLayout titulo="Aprobación de Presupuestos" filtrosTexto={filtrosTexto} printTable={printTable}>
      <div className="flex flex-wrap gap-3 items-end mb-2">
        <div className="space-y-1"><Label>Desde</Label><Input type="date" value={desde} onChange={e => setDesde(e.target.value)} className="w-36" /></div>
        <div className="space-y-1"><Label>Hasta</Label><Input type="date" value={hasta} onChange={e => setHasta(e.target.value)} className="w-36" /></div>
        <Button onClick={buscar} disabled={loading} className="bg-blue-600 hover:bg-blue-700 text-white">{loading ? "Buscando…" : "Consultar"}</Button>
      </div>

      {buscado && (
        <>
          {total > 0 && <p className="text-sm font-medium text-green-700">Tasa de aprobación: {tasaAprobacion}%</p>}
          <div className="border rounded overflow-hidden text-sm">
            <div className="grid grid-cols-4 bg-gray-100 font-semibold text-gray-600 text-xs px-3 py-2">
              <div>Estado</div><div className="text-center">Cantidad</div><div className="text-center">%</div><div className="text-right">Monto total</div>
            </div>
            {rows.map((r, i) => (
              <div key={i} className={`grid grid-cols-4 px-3 py-2 border-t ${i % 2 === 1 ? "bg-gray-50" : ""}`}>
                <div className="font-medium">{LABEL[r.estado] ?? r.estado}</div>
                <div className="text-center font-bold">{r.cantidad}</div>
                <div className="text-center text-gray-500">{r.porcentaje}%</div>
                <div className="text-right">{formatCurrency(r.monto)}</div>
              </div>
            ))}
            {rows.length === 0 && <div className="px-3 py-4 text-center text-gray-400">Sin datos</div>}
            <div className="grid grid-cols-4 px-3 py-2 border-t bg-gray-100 font-bold">
              <div>TOTAL</div><div className="text-center">{total}</div><div className="text-center">100%</div>
              <div className="text-right">{formatCurrency(rows.reduce((s, r) => s + r.monto, 0))}</div>
            </div>
          </div>
        </>
      )}
    </InformeLayout>
  );
}
