...
import { AlertTriangle, Search, Package, Printer } from "lucide-react";
...
export default function StockBajoPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      const res = await fetch("/api/repuestos?stockBajo=true");
  ...
  const filtered = repuestos.filter(r =>
    ...
    (r.numeroParte ?? "").toLowerCase().includes(q.toLowerCase()) ||
    (r.marca?.nombre ?? "").toLowerCase().includes(q.toLowerCase())
  );

  const sinStock    = filtered.filter(r => r.stockActual === 0);
  const bajosNoZero = filtered.filter(r => r.stockActual > 0 && r.stockActual <= r.stockMinimo);
    ...
    <div id="print-portal" style={{ display: "none", fontFamily: "Arial, sans-serif", fontSize: "11pt", color: "#000" }}>
      <style>{`
        @media print {
          ...
          #print-portal h2 { font-size: 13pt; margin: 14px 0 5px; }
      ...
      <h1 style={{ fontSize: "13pt", fontWeight: "bold", marginBottom: "3px" }}>Repuestos con Stock Bajo / Sin Stock</h1>
      <p style={{ fontSize: "9pt", color: "#555", marginBottom: "8px" }}>
        Fecha de emisión: {fechaHoy} &nbsp;·&nbsp; Total: {filtered.length} repuesto{filtered.length !== 1 ? "s" : ""}
        {sinStock.length > 0 && ` · Sin stock: ${sinStock.length}`}
        {bajosNoZero.length > 0 && ` · Stock bajo: ${bajosNoZero.length}`}
      </p>
      <hr style={{ borderTop: "1px solid #aaa", marginBottom: "10px" }} />

      {sinStock.length > 0 && (
        <>
          <h2>Sin Stock ({sinStock.length})</h2>
          <PrintTable rows={sinStock} tipo="sin" />
        </>
      )}
      {bajosNoZero.length > 0 && (
        <>
          <h2 style={{ marginTop: "16px" }}>Stock Bajo ({bajosNoZero.length})</h2>
          <PrintTable rows={bajosNoZero} tipo="bajo" />
        </>
      )}
    </div>
  );
    ...
    <>
      ...
      <div className="p-4 md:p-6 space-y-4 max-w-5xl">
        <div className="flex items-start justify-between flex-wrap gap-3">
          <div>
            <h1 className="text-2xl font-bold flex items-center gap-2">
              ...
              Repuestos con Stock Bajo / Sin Stock
            </h1>
            <p className="text-gray-500 text-sm mt-1">
              {loading ? "Cargando..." : `${repuestos.length} repuesto${repuestos.length !== 1 ? "s" : ""} con stock insuficiente`}
            </p>
          </div>
        ...
        <div className="relative w-72">
          ...
          <Input
            className="pl-9"
            placeholder="Buscar descripción, parte, marca..."
            value={q}
        ...
        {loading ? (
          ...
          <div className="text-center py-16 text-gray-400">
            ...
            <p>{repuestos.length === 0 ? "No hay repuestos con stock bajo" : "Sin resultados"}</p>
          </div>
        ) : (
          <div className="space-y-6">
            {sinStock.length > 0 && (
              <section>
                <h2 className="text-sm font-semibold uppercase text-red-600 mb-2 flex items-center gap-1">
                  <span className="inline-block w-2 h-2 rounded-full bg-red-600" />
                  Sin Stock ({sinStock.length})
                </h2>
                <ScreenTable rows={sinStock} tipo="sin" />
              </section>
            )}
            {bajosNoZero.length > 0 && (
              <section>
                <h2 className="text-sm font-semibold uppercase text-amber-600 mb-2 flex items-center gap-1">
                  <span className="inline-block w-2 h-2 rounded-full bg-amber-500" />
                  Stock Bajo ({bajosNoZero.length})
                </h2>
                <ScreenTable rows={bajosNoZero} tipo="bajo" />
              </section>
            )}
          </div>
        )}
...
function PrintTable({ rows, tipo }: { rows: Repuesto[]; tipo: "sin" | "bajo" }) {
  return (
    <table>
      <thead>
        <tr>
      ...
      <tbody>
        {rows.map(r => (
          <tr key={r.id} className={tipo === "sin" ? "sin-row" : "bajo-row"}>
            <td>{r.descripcion}</td>
            <td>{r.numeroParte ?? "—"}</td>
            <td>{r.marca?.nombre ?? "—"}</td>
            <td>{r.categoria?.nombre ?? "—"}</td>
            <td className="center" style={{ fontWeight: "bold", color: tipo === "sin" ? "#c00" : "#b45000" }}>{r.stockActual}</td>
...
function ScreenTable({ rows, tipo }: { rows: Repuesto[]; tipo: "sin" | "bajo" }) {
  return (
    <div className="border rounded-lg overflow-hidden">
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y">
          {rows.map(r => (
            <tr key={r.id} className={tipo === "sin" ? "bg-red-50 dark:bg-red-950/20" : "bg-amber-50 dark:bg-amber-950/20"}>
              <td className="px-3 py-2 font-medium">
                <Link href={`/repuestos/${r.id}`} className="hover:underline text-primary">
                  {r.descripcion}
                </Link>
              </td>
              ...
              <td className="px-3 py-2 text-center">
                <span className={`font-bold ${tipo === "sin" ? "text-red-600" : "text-amber-600"}`}>{r.stockActual}</span>
              </td>
